...
def holes(imgs, gt_imgs, num=1, s=20):
    """
    Add random `num` black squares to images of size `s`×`s`.
    """
    new_imgs = []
    for img in imgs:
...
def augment_data(imgs, gt_imgs):
    """
    Augment data with rotations, flips, contrast, holes. Less memory intensive than huge_augment_data
    """
...
def huge_augment_data(imgs, gt_imgs):
    """
    Augment data with rotations, flips, contrast, holes. Memory intensive!
    """
    imgs_90, gt_imgs_90 = rot90(imgs, gt_imgs)
    imgs_180, gt_imgs_180 = rot180(imgs, gt_imgs)
    imgs_270, gt_imgs_270 = rot270(imgs, gt_imgs)
